        ...
        <div className="flex flex-wrap gap-4">
            {images.map((img, index) => (
                <div key={index} className="relative">
                    ...
                    />
                    {img === mainImage && <div className="absolute inset-0"></div>}
                </div>
            ))}
        </div>
    );
...
const ProjectDetail = () => {
    ...
    const project = projects.find(
        (data) => data.name.toLowerCase().replace(/\s/g, "-") === id && data.category.toLowerCase() === category
    );
    ...
    return (
        <>
            ...
            <section className="flex justify-center mb-20">
                <div>
                    <div className="fixed h-[500px] ">
                        <div className="mb-4">
                            <img src={mainImage} alt="" className="w-[350px] h-[350px] rounded-lg object-cover" />
                        </div>
                        <Gallery images={project.img} onImageClick={handleImageClick} mainImage={mainImage} />
                    </div>
                    <div className="w-[500px] relative ml-[400px]">
